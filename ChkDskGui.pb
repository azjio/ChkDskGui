...
; 	HideWindow(#Window, #True)
	...
	EndIf
	hListView = ListIconGadget(#LIG, 5, 5, cs\w - 10, cs\h - 90, "-Диск-", 60, LV_Style)
	...
	AddGadgetColumn(#LIG, 1, "- № -", 50)
	AddGadgetColumn(#LIG, 2, "-Тип-", 52)
	AddGadgetColumn(#LIG, 3, "- Метка диска -", 125)
	AddGadgetColumn(#LIG, 4, "-FS-", 55)
	AddGadgetColumn(#LIG, 5, "Размер", 100)
	AddGadgetColumn(#LIG, 6, "     - Имя - ", 230)
	AddGadgetColumn(#LIG, 7, "-", 55)
	AddGadgetColumn(#LIG, 8, "- Занято -", 90)
...
;- ├ CheckBox / Button
	CheckBoxGadget(#chF, 10, cs\h - 80, cs\w - 190, 20, "/F - Исправление ошибок на диске") : SetGadgetState(#chF, #PB_Checkbox_Checked)
	CheckBoxGadget(#chR, 10, cs\h - 60, cs\w - 190, 20, "/R - Восстановление поврежденных секторов")
	CheckBoxGadget(#chX, 10, cs\h - 40, cs\w - 190, 20, "/X - Принудительное отключение тома") : SetGadgetState(#chX, #PB_Checkbox_Checked)
	HyperLinkGadget(#StatusBar, 20, cs\h - 17, cs\w - 200, 17, "", $FF0000) ; строка состояния
	CheckBoxGadget(#chAll, cs\w - 179, cs\h - 80, 170, 20, "Выделить все диски")
	...
	ButtonGadget(#btnStart, cs\w - 110, cs\h - 52, 100, 42, "Старт")
	...
	AddGadgetToolTip(#StatusBar, "Скопировать в буфер обмена для Win+R", 300, 0)
	AddGadgetToolTip(#btnMenu, "Меню", 300, 0)
	; 	AddGadgetToolTip(5, "Справка по ключам chkdsk.exe", 300, 0)
	AddGadgetToolTip(#btnStart, "Проверка дисков сейчас", 300, 0)
	; 	AddGadgetToolTip(7, "Импорт в реестр для проверки" + #CRLF$ + "во время загрузки системы", 300, 0)
	AddGadgetToolTip(#chAll, "Физические диски лучше" + #CRLF$ + "в разных потоках выполнить" + #CRLF$ + "экономя время", 300, 0)
...
;- ├ Menu
	If CreatePopupMenu(#Menu0) ; Создаёт всплывающее меню
		...
		MenuItem(#mComLineFull, "Полную для Win+R" + #TAB$ + "Ctrl+Shift+C")
		MenuItem(#mComLineBrief, "Краткую для Win+R")
		MenuItem(#mBatFile, "Для bat-файла")
		CloseSubMenu()
		OpenSubMenu("Реестр")
		MenuItem(#mCheckingSel, "Проверка выбранных при загрузке ОС")
		MenuItem(#mBootExecute, "Посмотреть BootExecute в реестре")
		MenuItem(#mEventvwr, "eventvwr.exe (лог проверки)")

		fMenuDisk = RegExistsKey()
		If fMenuDisk
			MenuItem(#mDiscMenu, "Удалить ChkDskGui в меню дисков")
		Else
			MenuItem(#mDiscMenu, "Добавить ChkDskGui в меню дисков")
		EndIf
		CloseSubMenu()
		MenuItem(#mCreateINI, "Создать ini")
		MenuItem(#mHelpChkdsk, "Справка chkdsk.exe" + #TAB$ + "F2")
		MenuItem(#mHelpGUI, "Справка ChkDskGui" + #TAB$ + "F1")
	...
	If FileSize(GetPathPart(ProgramFilename()) + "ChkDskGui.chm") < 1
		...
		SetMenuItemText(#Menu0, #mHelpGUI, "Скачать справку ChkDskGui")
	EndIf
	If CreatePopupMenu(#Menu1) ; Создаёт всплывающее меню
		MenuItem(#mOpen, "Открыть в Проводнике")
		MenuItem(#mDirty, "Проверить грязный бит")
	EndIf
	If Not Admin
		SetMenuItemText(#Menu0, #mCheckingSel, "Проверка выбранных при загрузке ОС" + " " + "(админ)")
		DisableMenuItem(#Menu0, #mCheckingSel, 1)
		If fMenuDisk
			SetMenuItemText(#Menu0, #mDiscMenu, "Удалить ChkDskGui в меню дисков" + " " + "(админ)")
		Else
			SetMenuItemText(#Menu0, #mDiscMenu, "Добавить ChkDskGui в меню дисков" + " " + "(админ)")
		EndIf
		DisableMenuItem(#Menu0, #mDiscMenu, 1)
		SetMenuItemText(#Menu1, #mDirty, "Проверить грязный бит" + " " + "(админ)")
				...
				Select EventMenu()    ; получим кликнутый элемент Меню...
					...
					Case #mHelpGUI
						...
						If FileSize(res$) > 11
							...
							; SetMenuItemText(#Menu0, 7, "Справка ChkDskGui")
						Else
							; DisableMenuItem(#Menu0, 7, 1)
							If MessageRequester("Скачать справку ChkDskGui", "Продолжить?", #PB_MessageRequester_YesNo) = #PB_MessageRequester_Yes
					...
					Case #mOpen
						...
						EndIf
					Case #mAbout
						If MessageRequester("О программе", "Автор AZJIO" + #CRLF$ + "Версия 4.3.3 от 17.04.2026" + #CRLF$ + #CRLF$ + "Хотите посетить тему обсуждения" + #CRLF$ + "и узнать об обновлениях?", #MB_OKCANCEL) = #IDOK
...
Procedure RegToMenuDisk()
	...
	If RegExistsKey() ; если существует, то удаляем
		If #ERROR_SUCCESS = RegDeleteKey_(#HKEY_CLASSES_ROOT, "Drive\shell\ChkDskGui\command")
			If #ERROR_SUCCESS = RegDeleteKey_(#HKEY_CLASSES_ROOT, "Drive\shell\ChkDskGui")
				...
				SetMenuItemText(#Menu0, #mDiscMenu, "Добавить ChkDskGui в меню дисков")
			EndIf
		EndIf
	Else ; иначе добавляем
		If #ERROR_SUCCESS = RegCreateKeyEx_(#HKEY_CLASSES_ROOT, "Drive\shell\ChkDskGui", 0, 0, #REG_OPTION_NON_VOLATILE, #KEY_ALL_ACCESS, 0, @hKey, @KeyInfo)
			...
			valie = #DOUBLEQUOTE$ + ProgramFilename() + #DOUBLEQUOTE$
		...
		If #ERROR_SUCCESS = RegCreateKeyEx_(#HKEY_CLASSES_ROOT, "Drive\shell\ChkDskGui\command", 0, 0, #REG_OPTION_NON_VOLATILE, #KEY_ALL_ACCESS, 0, @hKey, @KeyInfo)
			valie = #DOUBLEQUOTE$ + ProgramFilename() + #DOUBLEQUOTE$ + " " + #DOUBLEQUOTE$ + "%1" + #DOUBLEQUOTE$
			RegSetValueEx_(hKey, @"", 0, #REG_SZ, @valie, StringByteLength(valie, #PB_Unicode))
			RegCloseKey_(hKey)
			SetMenuItemText(#Menu0, #mDiscMenu, "Удалить ChkDskGui в меню дисков")
		EndIf
	EndIf
EndProcedure
...
Procedure Add_item_LV_Mask(Mask.l)
	...
	align_col_LV()
	align_Windows()
	SetWindowTitle(#Window, "ChkDskGui (подключен " + title + ")")
...
Procedure Del_item_LV(Mask.l)
	...
	For k = Count - 1 To 0 Step -1
		...
		EndIf
	Next
	SetWindowTitle(#Window, "ChkDskGui (отключен " + title + ")")
...
Procedure.s ComboListDrive(Drive2$)
	...
	Select type
		Case #DRIVE_REMOVABLE
			Drive2$ + ":" + #LF$ + "[" + DriveGetNumber(Drive2$ + ":") + "]" + #LF$ + "Rem  "
		Case #DRIVE_FIXED
			Drive2$ + ":" + #LF$ + "[" + DriveGetNumber(Drive2$ + ":") + "]" + #LF$ + "Fixed"
